...
> canónica ([`capabilities/agents/qa-analyst/AGENT.md`](../capabilities/agents/qa-analyst/AGENT.md))
> usa nombres genéricos (`read`/`search`/`edit`/`execute`) y 3 herramientas de Atlassian
> para comentar en el ticket. Para comentar desde Claude Code, agregar esas mismas
> herramientas en formato `mcp__<servidor>__<herramienta>`, nunca un wildcard. `Edit` y
> `Write` son solo para archivos de test.
>
