...
(depende de `../docs/architecture/BLOCKED-DECISIONS.md` #1) — es la mejor guía disponible
...
| **Evidence Contract, Evaluation Contract, Measurement Result Contract** (schema, no contenido) | Si cada equipo usa un formato distinto, nada es comparable ni auditable (`../docs/architecture/G4.3-Evidence-Evaluation-Measurement.md`) |
| **Taxonomía de Metrics** (las 7 categorías de `../metrics/framework.md`) | Mismo motivo — "common measurement model" |
| **Security baseline** — modelo de riesgo proporcional, reglas de MCP | `../docs/architecture/security-governance.md` |
...
| **Domain Skills** | El contenido de `azure-devops-cli`/`user-story` — mismo formato, contenido 100% distinto por dominio (`../docs/architecture/assessment-gate.md`) |
...
adelante — es tu autonomía, ya reconocida en `../docs/architecture/operating-model.md`.
