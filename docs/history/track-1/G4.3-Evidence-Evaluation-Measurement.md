...
sin infraestructura nueva) — y se elige un mecanismo de medición híbrido (referencia a una
herramienta de referencia interna de Baufest de métricas — no un equipo de MOA — cuando
esté disponible + archivo local versionado cuando no) que resuelve el bloqueo sin
construir una plataforma. Para la segunda, la investigación con `git log`
...
1. Adoption Test: paso "Measure" quedó **BLOCKED** — sin mecanismo accesible fuera de
   dicha herramienta de referencia.
...
| `metrics/framework.md`, `metrics/kpis.md` | Categorías y KPIs propuestos por el KO | No resuelven el caso de un equipo sin acceso a dicha herramienta de referencia — G4.3 lo cubre explícitamente |
...
duplicar dicha herramienta de referencia):

| Alternativa | Evaluación |
|---|---|
| A. Referencia a dicha herramienta de referencia existente | Válida cuando el equipo ya está cubierto por ese pipeline (Jira/ADO/Copilot/SonarQube) — pero **no se puede asumir disponible para todos** (ya confirmado: MOA Operaciones, Scato Puerto, Orquestador no tienen evidencia de estar conectados) |
...
están cubiertos por dicha herramienta de referencia (hoy: ningún caso confirmado con evidencia de estar en
...
| `source` | referencia a dicha herramienta de métricas, o `local measurement` (con quién/cómo) | Obligatorio |
...
Measurement Result (ejemplo estructural):
  ...
  source: local measurement (sin acceso confirmado a dicha herramienta de referencia para este caso)
...
| 10 | Measure | **BLOCKED** | **PARTIAL** | **Mejora real — deja de estar bloqueado.** El mecanismo D (sección 9) permite medir localmente sin dicha herramienta de referencia; sigue PARTIAL (no PASS) porque no hay baseline y la medición sigue siendo manual |
...
5. **Measurement Contract**: sección 10 — 13 campos, con reglas duras contra inventar
   valores o baselines.
6. **Mecanismo elegido para Measure**: combinación A+C — referencia a dicha herramienta
   de referencia cuando esté disponible, archivo local versionado con el mismo contrato
   cuando no.
