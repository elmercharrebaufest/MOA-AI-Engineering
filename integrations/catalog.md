...
| **MCP Atlassian (`com.atlassian/atlassian-mcp-server`)** | **CONFIGURATION VERIFIED / REAL USE REQUIRES VALIDATION** *(para las configuraciones wildcard encontradas en equipos)* | Referenciado en agents de DataAgro, Scato Logística y Orquestador (rama `feature/cardless4` en este último, **no integrada a `master`**), con scope `/*` en algunos casos (sin acotar a solo lectura). Sin evidencia de identidad, autenticación, ni ejecución real **para esas configuraciones wildcard específicas**. Ver [`../security/security-governance.md`](../security/security-governance.md) — prioridad de revisión de seguridad. **Actualización**: el patrón acotado (scope único `getJiraIssue`, CAP-008) **sí tiene 4 ejecuciones reales** en `MOA-AI-Engineering` — ver [`EXEC-20260908-004`](../records/jira-ARMOA277-191/EXEC-20260908-004/evidence.md) (issue tipo Error/Bug), [`EXEC-20260908-005`](../records/jira-ARMOA277-180/EXEC-20260908-005/evidence.md) (issue tipo Tarea/Task), [`EXEC-20260909-001`](../records/jira-ARMOA277-45/EXEC-20260909-001/evidence.md) (issue tipo Test/Xray, con actor de ejecución independiente — `PILOT-003`) y [`EXEC-20260917-001`](../records/jira-ARMOA277-194/EXEC-20260917-001/evidence.md) (issue tipo Historia/Story, con segundo actor de ejecución independiente) — pero esto no valida ni extiende a las configuraciones wildcard de los repos de equipo, que siguen `REQUIRES VALIDATION`. |
