...
| **`Action Type`** *(nuevo, agregado al implementar Context Acquisition & Resolution)* | `READ` / `ACT` / `BOTH` — ver [`security-governance.md`](../security/security-governance.md) para la frontera de contrato. **No se infiere de `Risk`**: READ no implica automáticamente bajo riesgo, ACT no implica automáticamente alto riesgo — la diferencia central es el impacto sobre estado externo, no un proxy de riesgo | `READ` para un Context Provider (ver CAP-007, CAP-008); `BOTH` para `azure-devops-cli` (pre-checks son READ, algunos subcomandos son ACT) |
| **`Context Requirements`** *(nuevo, agregado al implementar Context Acquisition & Resolution)* | Qué necesita la capability como input de contexto — `Direct Context`, `Resolved Context`, o ambos indistintamente (provider-agnostic). **No** se declara aquí de qué sistema viene el contexto — eso es responsabilidad del Context Provider, no de la capability consumidora | CAP-002: `Direct Context o Resolved Context, indistintamente` |
