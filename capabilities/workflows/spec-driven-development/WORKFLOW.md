...
| `research.md` / `contracts/` *(G8)* | Ambos opcionales, mismo criterio que el resto del Lite: solo si la feature real lo justifica. `research.md` con precedente real (ver sección "Artefactos ampliados") | Ambos opcionales, pero **`contracts/` rinde más acá**: el `reviewer`/`security-reviewer` puede verificar el código contra un contrato explícito en vez de contra su memoria del diseño — mismo rol que ya cumplen los contratos JSON *entre roles*, aplicado ahora a la interfaz *técnica* |
