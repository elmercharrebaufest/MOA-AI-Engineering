...
> `read-only-code-reviewer` (CAP-003) — cada equipo agrega su propio `model:` real al
> adoptar este Agent.
>
...
**Capability Registry**: [`CAP-012`](../../../registry/entries/product-owner.md).
...
[`user-story`](../../skills/user-story/SKILL.md) (CAP-002) — refinar un requerimiento
...
**No inventa una función nueva**: la lógica de refinamiento es exactamente la de CAP-002
...
- No lo uses como aprobador de que una historia está lista para Planning — la
  recomendación que produce es un insumo para que un humano decida, igual que CAP-002.
...
[`user-story`](../../skills/user-story/SKILL.md) (CAP-002), sin duplicar esa
documentación acá.
...
3. Aplicá exactamente las instrucciones de
   [`user-story`](../../skills/user-story/SKILL.md) (CAP-002) sobre el contenido real
...
- La lógica de refinamiento de [`user-story`](../../skills/user-story/SKILL.md) (CAP-002).
...
Obligatoria, sin excepción — mismo criterio que CAP-002: un PO o referente funcional debe
...
dominio, no por la función en sí (que ya está generalizada en CAP-002). La misma
...
CAP-003 (`read-only-code-reviewer`, sin `tools: edit`) y por CAP-008 (`jira-context`,
...
que reutiliza la lógica ya validada de CAP-002 y corrige el scope de acceso desde el día
...
dominio que adaptar, más allá de lo que ya requiere CAP-002 (tabla de roles reales).
