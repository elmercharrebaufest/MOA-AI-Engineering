...
| **Real Use Status** | **EXECUTED** *(7 ejecuciones registradas — 2 Direct Context: G4.4 y G4.6; 5 Connected Context: `EXEC-20260908-003/004/005`, `EXEC-20260909-001`, `EXEC-20260916-001`)*. Escala usada: `NOT FOUND / CONFIGURED / EXECUTED / VERIFIED`. **No es `VERIFIED`**: las 2 primeras ejecuciones (`EXEC-20260907-001` sobre MOA-1816, `EXEC-20260908-001` sobre MOA-1765) fueron CONTROLLED DRY-RUN con Direct Context; las 5 adicionales (`EXEC-20260908-003` Azure DevOps, `EXEC-20260908-004`/`EXEC-20260908-005`/`EXEC-20260909-001`/`EXEC-20260916-001` Jira/MCP) fueron con Connected Context vía Context Provider — las 7 fueron realizadas por el mismo agente que diseñó la capability (el campo `actor` de cada Evidence Record es genérico y no declara un desarrollador independiente de un equipo de MOA) — sigue sin haber confirmación humana independiente. G4.6 sumó una segunda ejecución Direct Context sobre un ticket real distinto específicamente para probar generalización; el vertical slice de Context Acquisition & Resolution sumó 4 más para probar el modo Connected Context sobre tipos de issue Jira distintos (`Error`/Bug, `Tarea`/Task, `Test`/Xray); `EXEC-20260916-001` sumó un cuarto tipo (`Historia`/Story, sin descripción) | Configuration Status sigue sin usarse para inferir esto — es evidencia directa de 7 ejecuciones reales registradas, distintas de la configuración |
