...
| **Purpose** | Resolver una referencia a un Work Item/PR de Azure DevOps hacia un `Resolved Context` consumible por cualquier capability, sin que la capability necesite saber que el origen fue Azure DevOps | PROPOSAL — patrón nuevo, generalizado a partir de evidencia real de CAP-008 |
...
| **Origin** | Generalización de la evidencia real de `azure-devops-cli` (CAP-008) — no una nueva instancia encontrada en un repo, es un patrón extraído en esta actividad | PROPOSAL |
...
| **Data** | Metadata de Work Items/PRs — no datos de negocio de aplicación | INFERENCE, igual que CAP-008 |
| **Data Classification** | REQUIRES VALIDATION | Política no existe (`BLOCKED-DECISIONS.md` #3) |
| **Tools** | `az cli` + extensión `azure-devops` — mismo mecanismo que CAP-008, sin herramienta nueva | FACT |
...
(Prioridad 3 de la fase de implementación). Extrae únicamente el subconjunto de CAP-008
relevante para resolución de contexto — no reemplaza ni duplica CAP-008, que sigue siendo
